...
import { HomeIcon, ClipboardList, User, LogOut, Menu, X } from "lucide-react";
...
export function Sidebar({ open, onOpenChange }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 lg:z-auto w-72 lg:w-64 bg-linear-to-b from-slate-900 to-slate-950 border-r border-indigo-500/30 p-4 lg:p-6 flex flex-col justify-between h-screen lg:h-auto lg:min-h-full transition-transform duration-200 lg:transform-none ${menuOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        {/* Topo */}
        ...
        {/* Logout */}
        <Button
          className="mt-8 flex items-center gap-2 justify-start w-full bg-linear-to-r from-indigo-600 to-indigo-700 text-white hover:from-indigo-500 hover:to-indigo-600 transition shadow-lg"
